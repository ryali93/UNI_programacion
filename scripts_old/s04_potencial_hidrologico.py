...
FLOW_ACC_MIN = os.path.join(RASTER_DIR, "facc_08_cnmin_q05.tif")  # Acumulacion de flujo para mes 8 (agosto)
...
def first_version(river_split, output):
    ...
    LISTA_Q = [[os.path.join(RASTER_DIR, "facc_" + str(x).zfill(2) + "_cnmin_q05.tif"), "Q_" + str(x).zfill(2)] for x in range(1, 13)]
    print(LISTA_Q)
...
def main():
    ...
    for distancia in lista_distancia:
        ...
        split_river(RED_HIDRICA, POINTS_RED_HIDRICA, SPLIT_RED_HIDRICA)
        PUNTOS_FIRST = os.path.join(SHP_DIR, "gpt_first_{}_cnmin_q05.shp".format(distancia))
        TABLA_FIRST = os.path.join(XLS_DIR, "tb_first_{}_cnmin_q05.xls".format(distancia))
