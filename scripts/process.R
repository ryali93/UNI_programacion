...
cn = raster("data/raster/ra_cn_max.tif")
...
for(i in seq(8,9)){
  ...
  pathcn   = paste0(path_basin, "/ra_cn_max.tif")
  ...
  for (j in seq(1,12)){
    ...
    writeRaster(q_st, paste0(path_basin, "/q_", formatC(j, width=2, flag="0"), "_cnmax_q95.tif"), overwrite = TRUE)
  }
  # guardar archivos
...
extract_data_serie = function(i, row){
  ...
  # facc = stack(list.files(paste0("E:/2020/uni/process/", as.character(i)), pattern = "(facc_[0,1][0-9]_cnmax_q95).tif$", full.names = T))
...
# df = extract_data_serie(4, 15) #df = extract_data_serie(4, c(15, 16))
# df = extract_data_serie(5, 5)
# df = extract_data_serie(6, 14)
# df = extract_data_serie(7, 8)
# df = extract_data_serie(8, 9) #df = extract_data_serie(8, c(9, 12))
df = extract_data_serie(9, 10) # df = extract_data_serie(9, c(10, 18))
# df = extract_data_serie(10, 7)
colnames(df)[1] = "mes"
colnames(df)[2] = "cnmin_qmean"
colnames(df)[3] = "cnmin_q95"
colnames(df)[4] = "cnmax_q95"
colnames(df)[5] = "cnmax_qmean"
# colnames(df)[5] = "cnmin_q95_QN403"
# colnames(df)[6] = "cnmax_qmean_Condorcerro"
# colnames(df)[7] = "cnmax_qmean_QN403"
# colnames(df)[5] = "cnmin_q05"

df1 = cbind(df, estacion = as.vector(xls$SanchezCerro))
df2 = cbind(df, estacion = as.vector(xls$QN2701))
df3 = cbind(df, estacion = as.vector(xls$Puchaca))
df4 = cbind(df, estacion = as.vector(xls$QN603))
df5 = cbind(df, estacion = as.vector(xls$Batan))
df6 = cbind(df, estacion = as.vector(xls$QN501))
df7 = cbind(df, estacion = as.vector(xls$Huacapongo))
df8 = cbind(df, estacion = as.vector(xls$Condorcerro))
df9 = cbind(df, estacion = as.vector(xls$Yanapampa))
df10 = cbind(df, estacion = as.vector(xls$Quirihuac)) 
...
nombre = "Cuenca Piura"
mes = seq(as.Date('2019-01-01'), as.Date('2019-12-01'), by = 'month')
grupo = c(rep("cnmin_qmean", 12), rep("cnmin_q95", 12), rep("estacion", 12))
datos = c(df1$cnmin_qmean, df1$cnmin_q95, df1$cnmax_qmean, df1$estacion)
dataf = data.frame(mes, datos, grupo)
val_max = (as.integer(max(datos) / 20) + 1) * 20
Sys.setlocale(category = 'LC_ALL', locale = 'english')
grafico = ggplot(dataf, aes(x=mes, y=datos, shape = grupo, col = grupo)) +
  geom_line(size = 1) + theme_bw() + 
  theme(plot.title = element_text(face='bold')) +
  ...
  scale_y_continuous(breaks = seq(0, val_max, 20), limits = c(0, val_max)) +
  geom_point(size = 3.5)
ggsave("process/1/img/caudales.png", width = 250, height = 180, units = "mm", dpi = 300)

## CUENCA CHIRA
nombre = "Cuenca Chira"
mes = seq(as.Date('2019-01-01'), as.Date('2019-12-01'), by = 'month')
grupo = c(rep("cnmin_qmean", 12), rep("cnmin_q95", 12), rep("estacion", 12))
datos = c(df2$cnmin_qmean, df2$cnmin_q95, df2$estacion)
dataf = data.frame(mes, datos, grupo)
val_max = (as.integer(max(datos) / 20) + 1) * 20
Sys.setlocale(category = 'LC_ALL', locale = 'english')
grafico = ggplot(dataf, aes(x=mes, y=datos, shape = grupo, col = grupo)) +
  ...
  scale_y_continuous(breaks = seq(0, val_max, 20), limits = c(0, val_max)) +
  geom_point(size = 3.5)
ggsave("process/2/img/caudales.png", width = 250, height = 180, units = "mm", dpi = 300)

## CUENCA MOTUPE
nombre = "Cuenca Motupe"
mes = seq(as.Date('2019-01-01'), as.Date('2019-12-01'), by = 'month')
grupo = c(rep("cnmin_qmean", 12), rep("cnmin_q95", 12), rep("estacion", 12))
datos = c(df3$cnmin_qmean, df3$cnmin_q95, df3$estacion)
dataf = data.frame(mes, datos, grupo)
val_max = (as.integer(max(datos) / 20) + 1) * 20
Sys.setlocale(category = 'LC_ALL', locale = 'english')
grafico = ggplot(dataf, aes(x=mes, y=datos, shape = grupo, col = grupo)) +
...
nombre = "Cuenca CHANCAY LAMBAYEQUE"
mes = seq(as.Date('2019-01-01'), as.Date('2019-12-01'), by = 'month')
grupo = c(rep("cnmin_qmean", 12), rep("cnmin_q95", 12), rep("estacion", 12))
datos = c(df4$cnmin_qmean, df4$cnmin_q95, df4$estacion)
dataf = data.frame(mes, datos, grupo)
val_max = (as.integer(max(datos) / 20) + 1) * 20
Sys.setlocale(category = 'LC_ALL', locale = 'english')
grafico4 = ggplot(dataf, aes(x=mes, y=datos, shape = grupo, col = grupo)) +
  geom_line(size = 1) + theme_bw() + 
  theme(plot.title = element_text(face='bold')) +
  scale_colour_manual(values = c('blue', 'red', 'green', 'brown')) +
  ylab(label = 'Q [m3/s]') +  xlab(label = '') +
...
mes = seq(as.Date('2019-01-01'), as.Date('2019-12-01'), by = 'month')
grupo = c(rep("cnmin_qmean", 12), rep("cnmin_q95", 12), rep("estacion", 12))
datos = c(df5$cnmin_qmean, df5$cnmin_q95, df5$estacion)
dataf = data.frame(mes, datos, grupo)
val_max = (as.integer(max(datos) / 20) + 1) * 20
Sys.setlocale(category = 'LC_ALL', locale = 'english')
grafico5 = ggplot(dataf, aes(x=mes, y=datos, shape = grupo, col = grupo)) +
  geom_line(size = 1) + theme_bw() + 
  theme(plot.title = element_text(face='bold')) +
  scale_colour_manual(values = c('blue', 'red', 'green', 'brown')) +
  ylab(label = 'Q [m3/s]') +  xlab(label = '') +
...
## CUENCA JEQUETEPEQUE
nombre = "Cuenca JEQUETEPEQUE"
mes = seq(as.Date('2019-01-01'), as.Date('2019-12-01'), by = 'month')
grupo = c(rep("cnmin_qmean", 12), rep("cnmin_q95", 12), rep("estacion", 12))
datos = c(df6$cnmin_qmean, df6$cnmin_q95, df6$estacion)
dataf = data.frame(mes, datos, grupo)
val_max = (as.integer(max(datos) / 20) + 1) * 20
Sys.setlocale(category = 'LC_ALL', locale = 'english')
grafico6 = ggplot(dataf, aes(x=mes, y=datos, shape = grupo, col = grupo)) +
  geom_line(size = 1) + theme_bw() + 
  theme(plot.title = element_text(face='bold')) +
  scale_colour_manual(values = c('blue', 'red', 'green', 'brown')) +
  ylab(label = 'Q [m3/s]') +  xlab(label = '') +
...
mes = seq(as.Date('2019-01-01'), as.Date('2019-12-01'), by = 'month')
grupo = c(rep("cnmin_qmean", 12), rep("cnmin_q95", 12), rep("estacion", 12))
datos = c(df7$cnmin_qmean, df7$cnmin_q95, df7$estacion)
dataf = data.frame(mes, datos, grupo)
val_max = (as.integer(max(datos) / 20) + 1) * 20
Sys.setlocale(category = 'LC_ALL', locale = 'english')
grafico7 = ggplot(dataf, aes(x=mes, y=datos, shape = grupo, col = grupo)) +
  geom_line(size = 1) + theme_bw() + 
  theme(plot.title = element_text(face='bold')) +
  scale_colour_manual(values = c('blue', 'red', 'green', 'brown')) +
  ylab(label = 'Q [m3/s]') +  xlab(label = '') +
...
nombre = "Cuenca SANTA"
mes = seq(as.Date('2019-01-01'), as.Date('2019-12-01'), by = 'month')
grupo = c(rep("cnmin_qmean", 12), rep("cnmin_q95", 12), rep("cnmax_qmean", 12), rep("cnmax_q95", 12), rep("estacion", 12))
datos = c(df8$cnmin_qmean, df8$cnmin_q95, df8$cnmax_qmean, df8$cnmax_q95, df8$estacion)
dataf = data.frame(mes, datos, grupo)
val_max = (as.integer(max(datos) / 20) + 1) * 20
Sys.setlocale(category = 'LC_ALL', locale = 'english')
grafico8 = ggplot(dataf, aes(x=mes, y=datos, shape = grupo, col = grupo)) +
  geom_line(size = 1) + theme_bw() + 
  theme(plot.title = element_text(face='bold')) +
  scale_colour_manual(values = c('blue', 'red', 'green', 'brown', 'black')) +
  ylab(label = 'Q [m3/s]') +  xlab(label = '') +
  ggtitle(nombre, subtitle = 'from 1981 to 2017 - Q mean - CN min') + 
  theme(plot.title    = element_text(size=16),
        plot.subtitle = element_text(size=16),
        ...
        axis.title    = element_text(size=17)) +
  scale_x_date(date_labels = '%b', breaks = '1 month') +
  scale_y_continuous(breaks = seq(0, val_max, 50), limits = c(0, val_max)) +
  geom_point(size = 3.5)


## CUENCA Pativilca
nombre = "Cuenca PATIVILCA"
mes = seq(as.Date('2019-01-01'), as.Date('2019-12-01'), by = 'month')
grupo = c(rep("cnmin_qmean", 12), rep("cnmin_q95", 12), rep("cnmax_qmean", 12), rep("cnmax_q95", 12), rep("estacion", 12))
datos = c(df9$cnmin_qmean, df9$cnmin_q95, df9$cnmax_qmean, df9$cnmax_q95, df9$estacion)
dataf = data.frame(mes, datos, grupo)
val_max = (as.integer(max(datos) / 20) + 1) * 20
Sys.setlocale(category = 'LC_ALL', locale = 'english')
grafico9 = ggplot(dataf, aes(x=mes, y=datos, shape = grupo, col = grupo)) +
  geom_line(size = 1) + theme_bw() + 
  theme(plot.title = element_text(face='bold')) +
  scale_colour_manual(values = c('blue', 'red', 'green', 'brown', 'black')) +
  ylab(label = 'Q [m3/s]') +  xlab(label = '') +
...
mes = seq(as.Date('2019-01-01'), as.Date('2019-12-01'), by = 'month')
grupo = c(rep("cnmin_qmean", 12), rep("cnmin_q95", 12), rep("estacion", 12))
datos = c(df10$cnmin_qmean, df10$cnmin_q95, df10$estacion)
dataf = data.frame(mes, datos, grupo)
val_max = (as.integer(max(datos) / 20) + 1) * 20
Sys.setlocale(category = 'LC_ALL', locale = 'english')
grafico10 = ggplot(dataf, aes(x=mes, y=datos, shape = grupo, col = grupo)) +
  geom_line(size = 1) + theme_bw() + 
  theme(plot.title = element_text(face='bold')) +
  scale_colour_manual(values = c('blue', 'red', 'green', 'brown')) +
  ylab(label = 'Q [m3/s]') +  xlab(label = '') +
  ...
  theme(plot.title    = element_text(size=16),
        ...
        axis.title    = element_text(size=17)) +
  scale_x_date(date_labels = '%b', breaks = '1 month') +
  scale_y_continuous(breaks = seq(0, val_max, 10), limits = c(0, 55)) +
  geom_point(size = 3.5)
